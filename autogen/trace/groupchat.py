...
class GroupChat(_GroupChat):
    ...
    def append(self, message: Node[Dict], speaker_node: Agent):
        ...
        @trace_op('[GroupChat.append.process_message] Process a message before appending it to the group chat.', unpack_input=False)
    ...
    @trace_op('[GroupChat._prepare_and_select_agents.select_speaker_message] Prompt the user to select the next speaker.', unpack_input=False)
    ...
    @trace_op('[GroupChat.select_speaker_msg] Return the system message for selecting the next speaker.', unpack_input=False)
    ...
    def _prepare_and_select_agents(
        ...
        @trace_op('[GroupChat._prepare_and_select_agents.get_graph_eligible_agents] Get the eligible agents.', unpack_input=False)
        ...
        if self.speaker_selection_method.lower() == "manual":
            ...
            @trace_op('[GroupChat._prepare_and_select_agents.process_speaker_message] Process the speaker message.', unpack_input=False)
    ...
    def run_chat(
        ...
        for i in range(groupchat.max_round):
            ...
            for agent in groupchat.agents:
                @trace_op('[GroupChatManager.broadcast_message] Broadcast a message to all agents except the speaker.', unpack_input=False)
