...
assert feedback_content in optimizer.parameters[0]
assert all([feedback_content in v[0] for v in optimizer.parameters[0].feedback.values()])  # make sure feedback is propagated to the parameter
node = last_message
while True:
    assert all([ feedback_content in v[0] for v in node.feedback.values()])
