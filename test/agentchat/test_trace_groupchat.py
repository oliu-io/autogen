...
def test_func_call_groupchat():
    agent1 = trace(autogen.ConversableAgent)(
        "alice",
    ...
    group_chat_manager = trace(autogen.GroupChatManager)(groupchat=groupchat, llm_config=False)
    ...
    agent3 = trace(autogen.ConversableAgent)(
        "carol",
        human_input_mode="NEVER",
        llm_config=False,
...
def test_plugin():
    ...
    groupchat = autogen.GroupChat(agents=[agent1, agent2], messages=[], max_round=2)
    group_chat_manager = trace(autogen.ConversableAgent)(name="deputy_manager", llm_config=False)
